-- 004_wizard_services_and_goal.sql
--
-- IMPORTANT — reality check done before writing this file: the live
-- create_tenant() in the streamline-admin Supabase project (lpjwfjkgqpgydzozuusj)
-- is NOT what supabase/migrations/003_create_tenant.sql in this repo describes.
-- Two later migrations applied directly against the shared project —
-- "unify_create_tenant_for_b2" and "harden_create_tenant_grants_and_search_path"
-- — never landed a matching file in *this* repo (they most likely live in the
-- sibling streamline-admin repo, which owns Workflow B2). The live function
-- now also accepts p_prospect_id/p_source, upserts the business
-- (`on conflict (id) do update`) so B2 can re-run against an existing prospect,
-- fuzzy-matches p_niche into a fixed key for the legacy preset seeder, and sets
-- is_streamline_owned. This migration is written against that REAL current
-- definition (confirmed via pg_get_functiondef against the live project just
-- before writing this file), not the stale 003 file. If you're reading this
-- later: check the live definition again before assuming any local migration
-- file is the source of truth — this repo's history and the DB's history can
-- drift when two repos share one Supabase project.
--
-- What this migration adds, additively, on top of the real live function:
--   - p_services jsonb default null — the wizard's exact, owner-edited service
--     list. When given, inserted directly (idempotent: skipped if the business
--     already has services, same guard seed_service_presets() itself uses, so
--     re-running create_tenant for the same slug — e.g. a B2 retry — never
--     duplicates rows). When absent, falls back to the existing
--     seed_service_presets(v_slug, v_niche_key) call, unchanged, so nothing
--     about the manual/n8n path changes.
--   - p_goal text default null — free-text personalization signal from the
--     wizard's "what's your goal" step, stored on the new businesses.goal
--     column. Coalesced on conflict so an upsert never clobbers an existing
--     goal with null.
-- Both are appended after the existing p_source param with defaults, so this
-- is an in-place REPLACE — every existing caller keeps working unchanged.

-- ---------------------------------------------------------------------------
-- 1) Schema: a nullable, free-form personalization field. Additive only.
-- ---------------------------------------------------------------------------
alter table public.businesses
  add column if not exists goal text;

-- ---------------------------------------------------------------------------
-- 2) create_tenant(): the real live body (B2-unified, upserting), plus
--    p_services / p_goal handling.
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_email := coalesce(
    ...
    'owner+' || v_slug || '@streamline-automations.agency'
  );
  ...
  -- same "don't double-seed" guard seed_service_presets() uses), else the
...
-- Rollback:
--   alter table public.businesses drop column if exists goal;
--   -- then re-apply the live function body captured above, minus the
--   -- p_services/p_goal params and the goal/services additions.
